...
from .forms import LoginForm, TagForm, MovieForm, PreviewForm, PwdForm, AuthForm, RoleForm, \
    AdminForm
from ..models import Admin, Tag, Movie, Preview, User, Comment, Moviecol, Oplog, Adminlog, \
    Userlog, Auth, Role
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not admin.check_pwd(form.data['pwd']):
            flash('密码错误', 'err')
            return redirect(url_for('admin.login'))
        session['admin'] = form.data['name']
...
def comment_del(id=None):
    comment = User.query.filter_by(id=id).first_or_404()
...
# 收藏列表
@admin.route("/moviecol/<int:page>/", methods=['GET'])
@admin_login_req
def moviecol(page):
    if not page:
        page = 1
    ...
    return render_template('admin/moviecol.html', page_data=page_data)
...
def moviecol_del(id=None):
    ...
    db.session.commit()
    flash("删除收藏成功", 'ok')
    return redirect(url_for('admin.moviecol', page=1))
